...
data/arm_b/index.jsonl
    one accepted file per line, including reconstructed content, provenance,
    line count, and size band
...
class ArmBExtractor:
    def __init__(self, args: argparse.Namespace):
        ...
        self.seen_prs: set[tuple[str, str]] = set()
    ...
    def ensure_output_dir(self) -> None:
        ...
        existing = [
            path
            for path in (self.index_path, self.excluded_path, self.summary_path, self.state_path, self.log_path)
            if path.exists()
        ]
        if self.args.fresh:
    ...
    def load_resume_artifacts(self) -> None:
        ...
        if self.state_path.exists():
            ...
            if parse_int(payload.get("state_version")) == STATE_VERSION:
        ...
        logging.info(
            "Resume state loaded: records=%d excluded=%d remaining=%d state_file=%s",
            len(self.records),
            self.excluded_count,
    ...
    def normalize_existing_record(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        if patch_text:
            ...
            if reason or candidate is None:
                raise RuntimeError(f"legacy patch failed current filter: {reason}")
            ...
            patch_sha_value = candidate["patch_sha"]
        else:
            normalized_language = normalize_language(raw.get("language")) or language_from_path(path)
            if normalized_language is None:
                raise RuntimeError("cannot derive language")
            content = str(raw.get("content") or "")
            if not content:
                raise RuntimeError("missing content and patch")
            file_line_count = parse_int(raw.get("file_line_count"), line_count(content))
            size_band = str(raw.get("size_band") or file_size_band(file_line_count))
            if size_band == "unknown":
                raise RuntimeError("size band unknown")
            patch_lines = parse_int(raw.get("patch_lines"), file_line_count)
    ...
    def register_existing_record(self, record: dict[str, Any]) -> None:
        ...
        self.seen_prs.add((repo, pr_identifier))
    ...
    def write_summary_and_state(
        ...
        *,
        ...
        state_payload = {
            ...
            "seen_patch_shas": sorted(self.seen_patch_shas),
            "seen_file_keys": sorted(list(self.seen_file_keys)),
            "seen_prs": sorted(list(self.seen_prs)),
            "api_requests": self.total_api_requests(client),
    ...
    async def detect_repo_ai_signal(self, client: GitHubClient, repo: str, default_branch_hint: str | None) -> tuple[str | None, str | None]:
        ...
        for path in paths:
            ...
            if path.startswith(".cursor/rules"):
                signal = f"repo_config:{path}"
                break
            if path.startswith(".github/workflows/") and REPO_AI_WORKFLOW_PATTERN.search(path):
                signal = f"repo_workflow:{path}"
                break
            ...
            if signal is None:
                ...
                for workflow in workflows:
                    ...
                    if REPO_AI_WORKFLOW_PATTERN.search(workflow_path):
                        signal = f"repo_workflow:{workflow_path}"
                        break
    ...
    async def process_repo_turn(self, client: GitHubClient, repo: str, search_language: str, turn_budget: int) -> int:
        ...
        if self.repo_counts[repo] >= self.args.repo_cap:
            ...
            self.excluded_by_reason["repo_cap_reached"] += 1
            return 0
        ...
        if repo_signal:
            ...
            if (repo, reason) not in self.logged_repo_exclusions:
        ...
        while pages_processed < self.args.repo_pr_page_budget_per_turn:
            ...
            if cursor["pr_page"] > self.args.max_pr_pages_per_repo_total:
                ...
                self.excluded_by_reason["repo_pr_page_limit"] += 1
                break

            self.current_phase = "pulls"
            prs = await client.list_closed_prs(repo, cursor["pr_page"])
            cursor["pr_page"] += 1
            pages_processed += 1
            ...
            for pr in sorted(
                ...
                if pr_number <= 0:
                    continue
                pr_identifier = str(pr_number)
                if (repo, pr_identifier) in self.seen_prs:
                    continue
                self.seen_prs.add((repo, pr_identifier))
                ...
                for file_info in sorted(
                    ...
                    if status not in ALLOWED_STATUS:
                        self.excluded_by_reason["file_status_not_allowed"] += 1
                        continue
                    ...
                    if not self.compatible_output_language(search_language, output_language):
                        self.excluded_by_reason["file_language_not_in_search_family"] += 1
                        continue

                    cell_key = (output_language, candidate["size_band"])
                    if self.remaining_targets[cell_key] <= 0:
                        self.excluded_by_reason["target_already_satisfied"] += 1
                        continue
                ...
                if cursor["yielded_total"] == 0 and cursor["scanned_prs"] >= self.args.zero_yield_pr_threshold:
                    ...
                    self.zero_yield_repos.add(repo)
                    self.excluded_by_reason["zero_yield_repo"] += 1
                    break
